...
O runner `scripts/test-api-isolated.sh` também prepara MariaDB temporário a partir do ID da imagem Kairós existente, sem portas publicadas e sem volumes produtivos. `wordpress/tests/test-admin-gate-mariadb.php` usa a classe `wpdb` da imagem WordPress, SQL real e índice único InnoDB: 12 processos PHP aguardam uma barreira e disputam o mesmo nonce; exatamente um deve ganhar. Verifica replay posterior, nonce distinto, ausência de autoload, limpeza limitada a 256 registros, preservação de opções alheias, saturação e falha do banco. Hooks/diagnósticos WordPress são shims; não é E2E HTTP nem prova dos plugins ativos. O teste exige hostname/schema fixos de fixture antes de qualquer SQL e o runner remove os containers/dados temporários. Resultado de execução registrado na entrega; nenhum resultado é presumido pela existência do teste.

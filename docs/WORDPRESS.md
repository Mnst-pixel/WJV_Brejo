...
The approved human administrators are `Vinícius`, `João_Ribeiro`, and `Wanderson`. The learning application accepts the owner-facing spellings or the canonical internal usernames `vinicius`, `joao_ribeiro`, and `wanderson`; administrative access requires each person to enroll their own MFA factor after the password is accepted. The WordPress technical usernames are the same canonical lowercase identifiers, all with the `administrator` role. Placeholder `@kairos.invalid` addresses are used for the two additional accounts until the owner supplies individual delivery addresses. Legacy HTTP/IP entry points redirect to the canonical HTTPS login.

...
A limpeza remove somente o container cujo ID foi criado nesta execução e cujo label corresponde ao run, além do staging com caminho canônico/prefixo conferidos. Não remove volumes, bancos, redes ou backups anteriores. Não usa prune. Logs transitórios são privados e descartados; não imprimir dumps ou erros SQL em mensagens públicas.

...
Reading verifies the approval hash before returning content, recommending a title, or recording progress. The digest covers identity, revision number, all exposed legal fields and review metadata. Older candidate receipts require fresh human review rather than automatic re-signing. Dashboard results and formal admission serialize on the same account lock. Reading history is immutable under reconciled runtime grants.

Account administration never reveals credentials or recovery tokens. Privileged accounts remain protected across inactivation/grant expiry. Role PUT requires an optimistic version; profile/access/role changes revoke tracked sessions. Email uniqueness excludes empty addresses and ignores case. Recovery confirmation locks the account and rejects token replay, disabled accounts and malformed identifiers. SMTP exceptions preserve the generic public 202 response, while the authorized operator sees only delivery state.
...
Metas: o servidor rejeita progresso quantitativo fornecido pelo cliente, exige versão para PATCH e revalida conta/sessão sob lock. Criação usa recibo por titular; GET não altera fatos de estudo. Recuperação temporária não é fonte canônica. Ver [P4-METAS.md](P4-METAS.md), ainda candidato sem deploy.

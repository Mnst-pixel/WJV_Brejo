...
## Revalidação com metas —18/09/2026

O candidato **10089ff** passou em644API/268operações/13contratosGit em Linux, incluindo o teste de DML/restore atualizado para0017 e API de metas sob runtime restrito. Imagens API/parser/web do mesmo commit, build/smoke web e no-touchv2 PASS; hashes completos em [P4-METAS.md](P4-METAS.md).

Restore integral **PASS às03:29UTC**: backup novo `kairos-predeploy-20260918T032758Z-b0d63b51bc7b.tar.gz.enc`, evidência `kairos-restore-20260918T032812Z-2d9fb75a2a74.evidence`. **16migrations como migrator**, **64tabelas/90registros originais preservados** e rerun idempotente; matriz de autorização é a única transformação esperada. API editorial/notas/metas como runtime sem superusuário, ownership, replay, versão e rollback das linhas sintéticas PASS. Integridade/arquivos/PostgreSQL/MariaDB/MinIO, cleanup e comparador estrito v2 sem exceções PASS. Recibo local `goals-scoped-restore-20260918.json`.

Executores em `scripts/releases/verify-product-{image,web,recovery}-10089ff.sh`, commit **e7dae03**. Helpers somente da extração nova do archive verificado; B30probeslocaisPASS, limites de simulação dos metadadosLinux documentados. Não houve deploy. O ensaio não substitui manutenção/drenagem, rollback completo de release, smoke público ou ativação das credenciais segregadas.

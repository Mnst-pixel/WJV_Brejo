...
Personal study migrations0015–0017 add optimistic versions, creation receipts, flashcard review snapshots/scheduling, and quantitative goals. Retain these fields and histories. An old API is **not automatically compatible**: SQL inserts for Goal must supply the newly required columns, and the old UI cannot represent quantitative progress. Restore tests explicitly exercise the current schema and identify the progress constraint. Before traffic is reopened, rehearse the complete prior release with its consistent database/configuration recovery set; after new writes, preserve/reconcile them rather than restoring an earlier dump silently. See [P4-METAS.md](P4-METAS.md).

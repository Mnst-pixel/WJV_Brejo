...
### Restore integral do candidato10089ff — PASS

Em **18/09/2026 03:29UTC**, backup novo `/srv/kairos/backups/kairos-predeploy-20260918T032758Z-b0d63b51bc7b.tar.gz.enc` foi restaurado integralmente em recursos temporários. Evidência `/srv/kairos/backups/kairos-restore-20260918T032812Z-2d9fb75a2a74.evidence`; recibo local `goals-scoped-restore-20260918.json`.

**16migrations como migrator**, **64tabelas/90registros originais preservados**, repetição idempotente. Única alteração original permitida: matriz `core_permission_roles`, migrada e testada separadamente. Grants restritos e API editorial/notas/metas sob runtime sem superuser PASS; ownership/replay/versão PASS. Checksum/manifesto/arquivos/PostgreSQL/MariaDB/MinIO, cleanup e **no-touch v2 sem exceções PASS**. Chamador usa variáveis Docker propositalmente inválidas para provar que o restore usa o socket local fixo. Todos os dados sintéticos foram revertidos.

Executor versionado em **e7dae03**; código/imagens ensaiados continuam **10089ff**. O gate de restore da candidata está comprovado. Não prova rollback integral, compatibilidade do código anterior com schema novo, HTTP público ou ativação produtiva. Descriptor e credenciais preparados anteriormente para c12e53a permanecem inativos e não representam10089ff. Próxima etapa operacional: preparar descriptor/checkout/mounts desta revisão e fechar coordenador de manutenção/drenagem/cutover/rollback antes de liberar tráfego. SMTP/storageoff-host permanecem dependências externas, sem bloquear esse trabalho.

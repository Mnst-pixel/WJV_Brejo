...
base_tag="kairos-api:verified-base-${base_id#sha256:}"
...
docker build --pull=false --memory 1536m --memory-swap 1536m \
  --build-arg "BASE_IMAGE=$base_tag" --build-arg "SOURCE_REVISION=$revision" \
...
docker build --pull=false --network none --memory 512m --memory-swap 512m \
  --build-arg "KAIROS_API_BASE_IMAGE=$api_target" -f "$source_dir/services/parser/Dockerfile" \
